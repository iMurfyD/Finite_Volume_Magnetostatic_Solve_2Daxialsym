function fmag = calc_f_top_circum_method(sep, Hymag, susc,a,n,m)
...
xlim([-3 3]); ylim([-3 3]);
title('\Nabla \mu_x', 'interpreter', 'latex');
colorbar; 
...
%             if(norm([XX(ii,jj) YY(ii,jj)]'-r1)<=a)
            if(abs(XX(jj,ii)) <= mean([ds dz]))
                circum = 2*pi*mean([ds dz]);
            else
                circum = 2*pi*abs(XX(jj,ii))/2;
            end
            c_map(ii,jj) = circum;
