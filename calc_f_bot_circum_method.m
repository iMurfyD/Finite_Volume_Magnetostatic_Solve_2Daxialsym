function fmag = calc_f_bot_circum_method(sep, Hymag, susc,a,n,m)
...
sdom = linspace(-3*a, 3*a, n);
zdom = linspace(-4*a, 4*a, m);
ds = sdom(2)-sdom(1);
dz = zdom(2)-zdom(1);
...
mididx = round((topidx+botidx)/2);
circum = pi*abs(XX);
fy_per_vol_dV = FMY*ds*dz.*circum;
botsqrd = sum(fy_per_vol_dV(botidx:mididx,:),'all');
topsqrd = sum(fy_per_vol_dV(mididx+1:topidx,:),'all');
fmag = [botsqrd topsqrd];%sum(fy_per_vol_dV(botidx:topidx,:),'all');
